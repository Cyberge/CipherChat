...
(function () {
  ...
  function applyAvatarStyle(el, avatar) {
    const a = avatar || CC.generateAvatar("user");
    el.style.background = a.color || "#1e2736";
    el.style.color = "#fff";
  }
  ...
  async function getProfileCached(uid) {
    if (_profileCache[uid] && _profileCache[uid].avatar?.initials) return _profileCache[uid];
    try {
      const p = await CC.getUserProfile(uid);
      if (p) {
        if (!p.avatar?.initials) {
          p.avatar = CC.generateAvatar(p.username || uid);
        }
        _profileCache[uid] = p;
      }
      return p || null;
  ...
  function renderConversations() {
    ...
    filtered.forEach((c) => {
      ...
      applyAvatarStyle(av, c.avatar || CC.generateAvatar(c.name || (c.type === "group" ? "GR" : "DM")));
  ...
  async function openDM(otherUid, otherUsername) {
    ...
    applyAvatarStyle(chatAvatarEl, av);
  ...
  async function init() {
    ...
    CC.onAuthChange(async (user) => {
      ...
      applyAvatarStyle(meAvatarEl, av);
      ...
      CC.listenConversations(currentUser.uid, async (convos) => {
        ...
        for (const c of convos) {
          try {
            const p = await getProfileCached(c.otherUid);
            const username = p?.username || "unknown";
            const avatar = p?.avatar || CC.generateAvatar(username);
            dmConvos.push({
              type: "dm",
              id: c.id,
              otherUid: c.otherUid,
            ...
            console.warn("Skipping broken DM entry:", e.message);
          }
        }
        const groups = allConvos.filter((x) => x.type === "group");
        allConvos = [...dmConvos, ...groups];
        renderConversations();
      });
